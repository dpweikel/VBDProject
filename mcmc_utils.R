...
make.logistic.mod.samps<-function(coda.samps, nchains=2, samp.lims=c(151, 5000), sig=TRUE, Y.0=FALSE){
  ...
  for(i in 1:nchains){
    if(Y.0){
      ...
      K<-c(K, coda.samps[[i]][l1:l2,1])
      r<-c(r, coda.samps[[i]][l1:l2,3])
      d<-c(r, coda.samps[[i]][l1:l2,3])
      m<-c(r, coda.samps[[i]][l1:l2,3])
      if(sig) sigma<-c(sigma, coda.samps[[i]][l1:l2,4])
    }
    else{
      K<-c(K, coda.samps[[i]][l1:l2,1])
...
make.linear.samps<-function(coda.samps, nchains=2, samp.lims=c(151, 5000), sig=TRUE){
  ...
  if(sig){
    samps<-data.frame(matrix(c(inter, slope, sigma), ncol=3, byrow=FALSE))
    names(samps)<-c("inter", "slope", "sigma")
  }
  else{
    samps<-data.frame(matrix(c(inter, slope), ncol=2, byrow=FALSE))
    names(samps)<-c("inter", "slope", "sigma")
...
make.sims.temp.resp<-function(sim="quad", samps, Temps, thinned, p.name="PDR"){
  ...
  for(i in 1:length(thinned)){
    ...
    #Added by Daniel W. for the linear possibility   
    if(sim=="linear"){
      inter<-as.numeric(samps$inter[i])
      slope<-as.numeric(samps$slope[i])
      data.sim[,i]<-linear(Temps, inter=inter, slope=slope)
      w<-which(data.sim[,i]<0)
      data.sim[w,i]<-0
    }
